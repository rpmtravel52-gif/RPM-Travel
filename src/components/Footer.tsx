import Link from 'next/link';

const WA = 'https://wa.me/6285282828005?text=Halo%20RPM%20Travel';

export default function Footer() {
  return (
    <footer className="bg-primary-900 text-white">
      {/* Gold top border */}
      <div className="h-1 bg-gradient-to-r from-transparent via-gold-500 to-transparent" />

      <div className="max-w-7xl mx-auto px-4 py-14 grid grid-cols-1 md:grid-cols-3 gap-10">
        {/* Brand */}
        <div>
          <div className="flex items-center gap-2 mb-4">
            <div className="w-9 h-9 bg-gold-500 rounded-xl flex items-center justify-center text-primary-900 font-bold text-lg">R</div>
            <div className="leading-tight">
              <div className="font-display font-bold text-white text-sm">RPM Travel</div>
              <div className="font-display font-bold text-gold-400 text-sm -mt-0.5">Curup, Bengkulu</div>
            </div>
          </div>
          <p className="text-gray-400 text-sm leading-relaxed max-w-xs mb-4">
            Jasa travel dan sewa bus wisata terpercaya di Curup. Melayani rute Curup–Palembang, Curup–Lebong, dan bus pariwisata.
          </p>
          <a
            href={WA}
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 bg-green-500/20 border border-green-500/30 text-green-400 text-sm font-semibold px-4 py-2 rounded-xl hover:bg-green-500/30 transition-colors"
          >
            <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
              <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
            </svg>
            0852-8282-8005
          </a>
        </div>

        {/* Layanan */}
        <div>
          <h4 className="text-gold-400 text-xs font-bold uppercase tracking-widest mb-4">Layanan</h4>
          <ul className="space-y-2.5">
            {[
              { href: '/travel-bengkulu-palembang', label: 'Travel Bengkulu – Palembang' },
      { href: '/travel-curup-palembang', label: 'Travel Curup – Palembang' },
              { href: '/travel-curup-lebong',    label: 'Travel Curup – Lebong' },
              { href: '/sewa-bus-wisata',        label: 'Sewa Bus Wisata' },
              { href: '/pesan',                  label: 'Pesan Tiket Online' },
            ].map((l) => (
              <li key={l.href}>
                <Link href={l.href} className="text-gray-400 hover:text-gold-400 text-sm transition-colors">
                  {l.label}
                </Link>
              </li>
            ))}
          </ul>
        </div>

        {/* Kontak */}
        <div id="kontak">
          <h4 className="text-gold-400 text-xs font-bold uppercase tracking-widest mb-4">Kontak & Lokasi</h4>
          <ul className="space-y-3 text-sm text-gray-400">
            <li className="flex gap-2">
              <span>📍</span>
              <span>Jl. S. Parman, Talang Benih, Curup, Rejang Lebong, Bengkulu 39119</span>
            </li>
            <li className="flex gap-2">
              <span>📱</span>
              <a href={WA} target="_blank" rel="noopener noreferrer" className="hover:text-gold-400 transition-colors">
                0852-8282-8005
              </a>
            </li>
            <li className="flex gap-2">
              <span>📱</span>
              <a href={WA} target="_blank" rel="noopener noreferrer" className="hover:text-gold-400 transition-colors">
                0852-8282-8006
              </a>
            </li>
            <li className="flex gap-2">
              <span>🕐</span>
              <span>06.00 – 22.00 WIB, Setiap Hari</span>
            </li>
            <li>
              <a
                href="https://share.google/nhQgSmsBuE5taP7H1"
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1.5 text-gold-400 hover:text-gold-300 text-sm font-medium transition-colors mt-1"
              >
                📍 Lihat di Google Maps →
              </a>
            </li>
          </ul>
        </div>
      </div>

      {/* Bottom bar */}
      <div className="border-t border-white/10">
        <div className="max-w-7xl mx-auto px-4 py-4 flex flex-col sm:flex-row justify-between items-center gap-2 text-xs text-gray-500">
          <p>© {new Date().getFullYear()} RPM Travel Curup. Hak Cipta Dilindungi.</p>
          <p>Jl. S. Parman, Talang Benih, Curup, Bengkulu 39119</p>
        </div>
      </div>
    </footer>
  );
}
