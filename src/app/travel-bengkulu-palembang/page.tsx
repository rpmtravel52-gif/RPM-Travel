...
export default function TravelBengkuluPalembangPage() {
  return (
    <>
      {/*
        FIX LCP: Hero section sekarang berisi gambar langsung di dalamnya.
        Sebelumnya gambar ada di section terpisah di bawah teks hero →
        browser memilih teks paragraf sebagai LCP element (lambat karena nunggu font).
        Sekarang gambar muncul di atas fold (mobile: atas teks, desktop: kanan teks)
      ...
      <section className="bg-primary-900 pt-10 pb-0 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto px-4 relative">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center pb-10">
            ...
            {/* Teks hero — desktop: kiri, mobile: bawah gambar */}
            ...
            {/* Gambar hero — mobile: tampil duluan (order-first), desktop: kanan */}
      ...
      <section className="bg-cream-100 border-b border-gray-200 py-8">
        <div className="max-w-4xl mx-auto px-4 grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            { label: 'Harga', value: 'Hubungi Kami' },
          ].map(item => (
            <div key={item.label} className="text-center">
              <p className="text-xs text-gray-400 uppercase tracking-wide mb-1">{item.label}</p>
              <p className="font-bold text-primary-900 text-sm">{item.value}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="py-14 bg-white">
        <div className="max-w-4xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
            {schedules.map(s => (
              <div key={s.jam} className="bg-primary-900 rounded-2xl p-6 flex items-center gap-4">
                <span className="text-4xl" aria-hidden="true">{s.icon}</span>
                <div>
                  <p className="text-gold-400 font-bold text-2xl">{s.jam}</p>
                  <p className="text-white font-semibold text-sm">{s.keterangan}</p>
