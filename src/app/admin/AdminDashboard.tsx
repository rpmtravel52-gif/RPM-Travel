...
// Normalkan nomor HP ke format internasional tanpa "+"
// Contoh: "08123..." → "628123..." | "628123..." → tetap
...
function OrderRow({ order, onRefresh }: { order: Order; onRefresh: () => void }) {
  ...
  // ── Pesan WA Invoice (info tagihan) ke pelanggan ─────────────
  ...
  // ── Pesan WA Tiket (konfirmasi lunas) ke pelanggan ───────────
  const buildWaTiket = () => {
    const tgl = new Date(order.tanggal).toLocaleDateString('id-ID', {
      day: '2-digit', month: 'long', year: 'numeric',
    });
  ...
  // WA lama → ke admin (koordinasi internal)
  ...
  return (
    <div className="bg-white/5 border border-white/10 rounded-xl p-5 relative hover:border-sky-500/30 transition-all">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3 text-xs">
        ...
        <div>
          <p className="text-slate-500 uppercase tracking-wide mb-0.5">Tanggal</p>
          <p className="text-white font-semibold">
            {new Date(order.tanggal).toLocaleDateString('id-ID', { day: '2-digit', month: 'short', year: 'numeric' })}
            {order.jam && <span className="text-slate-400"> · {order.jam} WIB</span>}
          </p>
        </div>
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-1 mb-3 text-xs">
        {order.hp && (
          <p className="text-slate-400">📱 +62{order.hp.replace(/^0|^62/, '')}</p>
        )}
        {order.email && (
          <p className="text-slate-400">📧 {order.email}</p>
        )}
        {!order.hp && !order.email && (
          <p className="text-slate-600 italic">* Tidak ada kontak pelanggan</p>
        )}
      </div>
      ...
      <div className="pt-3 border-t border-white/10">
        ...
        <div className="flex flex-wrap gap-2 mb-2">
          ...
          <button
            ...
            title={!order.email ? 'Email tidak tersedia' : 'Kirim invoice ke email pelanggan'}
          ...
          >
            {invoiceLoading ? '⏳' : '📄'} {invoiceLoading ? 'Mengirim...' : 'Email Faktur'}
          </button>
          <button
            onClick={handleSendTicket}
            disabled={order.status !== 'PAID' || ticketLoading || !order.email}
            title={order.status !== 'PAID' ? 'Konfirmasi lunas terlebih dahulu' : !order.email ? 'Email tidak tersedia' : ''}
          ...
          >
            {ticketLoading ? '⏳' : '🎫'} {ticketLoading ? 'Mengirim...' : 'Email Tiket'}
          </button>
        </div>

        {/* Baris 2: WA ke pelanggan */}
        <div className="flex flex-wrap gap-2">
          {/* WA Invoice ke pelanggan */}
          {waInvoiceUrl ? (
            <a
              href={waInvoiceUrl}
              target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center gap-1.5 bg-emerald-700/50 hover:bg-emerald-600/70 text-emerald-300 border border-emerald-600/40 rounded-lg text-xs px-3 py-1.5 font-semibold transition-all"
            >
            ...
            <span className="inline-flex items-center gap-1.5 bg-white/5 text-slate-600 border border-white/10 rounded-lg text-xs px-3 py-1.5 font-semibold cursor-not-allowed" title="Nomor HP pelanggan tidak tersedia">
              WA Invoice
            </span>
          )}

          {/* WA Tiket ke pelanggan — hanya aktif jika PAID */}
          {waTiketUrl && order.status === 'PAID' ? (
            <a
              href={waTiketUrl}
              target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center gap-1.5 bg-green-700/50 hover:bg-green-600/70 text-green-300 border border-green-600/40 rounded-lg text-xs px-3 py-1.5 font-semibold transition-all"
            >
              <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24">
                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
              </svg>
            ...
            <span
              className="inline-flex items-center gap-1.5 bg-white/5 text-slate-600 border border-white/10 rounded-lg text-xs px-3 py-1.5 font-semibold cursor-not-allowed"
          ...
          {/* WA ke admin (koordinasi internal) */}
          <a
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mt-8 bg-white/5 border border-white/10 rounded-xl p-4 text-xs text-slate-400">
          ...
          <div className="space-y-1">
            ...
            <p><span className="text-green-400">WA Tiket ✓</span> — Kirim konfirmasi lunas + tiket ke WhatsApp pelanggan (hanya aktif setelah LUNAS).</p>
